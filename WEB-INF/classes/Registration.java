...
public class Registration extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		{
			HashMap<String, User> users = MySqlDataStoreUtilities.selectUser();
			...
			else
			{
				...
				User user = new User(firstname,lastname,username,password,repassword,usertype);
	...
	protected void displayRegistration(HttpServletRequest request,
		...
		pw.print("<form method='post' action='Registration'>"
				...
				+ "</td></tr><tr><td>"
				+ "<h3>User Type</h3></td><td><select name='usertype' class='input'><option value='patient' selected>Patient</option><option value='doctor'>Doctor</option><option value='customerservice'>Customer Specialist</option></select>"				
				+ "</td></tr></table>"
